...
export function isValidPoolStatus(status) {
  const allowed = ["OPEN", "FUNDED", "EXPIRED"];
  return allowed.includes(status);
}

export function getSafeMatchBreakdown(raw) {
...
export function normalizePoolDoc(raw) {
...
export function validatePoolForWrite(pool) {
  const errors = [];
