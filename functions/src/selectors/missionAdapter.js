...
function normalizeMission(mission, options = {}) {
  const normalized = {
...
function filterMissionsByAirport(all = [], airportCode) {
  const normalizedAirportCode = normalizeAirportCode(airportCode);
  return all.filter((mission) => mission?.airport?.start === normalizedAirportCode);
}

function getRegionalMissions(all = [], airportCode, primary = []) {
  const normalizedAirportCode = normalizeAirportCode(airportCode);
...
function ensureThreeMissions({primary = [], regional = [], global = []}) {
  const result = [];
...
function buildGuaranteedMissionCards({airportCode, missions = []}) {
  const normalizedAirportCode = normalizeAirportCode(airportCode);
...
module.exports = {
  EMERGENCY_MISSIONS,
  FALLBACK_IMAGE,
  buildGuaranteedMissionCards,
  buildStatLine,
  dedupeMissions,
  ensureThreeMissions,
  filterMissionsByAirport,
  getRegionalMissions,
  mapMissionToCard,
