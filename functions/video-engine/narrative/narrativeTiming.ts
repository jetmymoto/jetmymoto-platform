export type FormatIntent = "hero" | "reel" | "ad" | "tour_trailer";

export type RuntimeAllocation = {
  totalSec: number;
  hookSec: number;
  ctaSec: number;
  actDurations: number[]; // seconds per act
};

function resolveTotals(format: FormatIntent): { totalSec: number; hookSec: number; ctaSec: number } {
  switch (format) {
    case "ad":
...
function weightsForActs(actCount: number): number[] {
  if (actCount <= 1) return [1];
  if (actCount === 2) return [0.45, 0.55];
  return [0.30, 0.40, 0.30];
}

export function allocateRuntime(format: FormatIntent, phaseCount: number): RuntimeAllocation {
