...
function log(event: Omit<LogEvent, "step">): void {
  const entry = { step: "step1", ...event } as LogEvent;
...
export type Step1Options = {
  keepLocalFrames?: boolean;   // default: false — clean up temp dir after upload
  buildArchive?: boolean;      // default: false — produce frames.tar.gz
  imageFormat?: ImageFormat;   // default: "png" — use "webp" for 60-70% smaller frames
  frameCount?: number;         // default: 150 — reduced-frame pipeline
  bucket: import("@google-cloud/storage").Bucket;
  localTempBaseDir?: string;   // default: ./temp_frames
};
...
export async function downloadFrames(
  ...
  const MAPBOX_TOKEN =
    process.env.REMOTION_MAPBOX_TOKEN || process.env.MAPBOX_TOKEN;
  if (!MAPBOX_TOKEN) throw new Error("[Step1] MAPBOX_TOKEN missing");
  ...
  console.log(
    `[Step1] Frame plan ready: ${framePlan.totalFrames} frames, ` +
  ...
  await concurrentMap(framePlan.frames, CONCURRENCY, async (entry) => {
    ...
    let attempts = 0;
    const wrappedFetch = async (): Promise<void> => {
      attempts++;
    ...
    if (entry.frameIndex % 100 === 0) {
      console.log(
        `[Step1] Frame ${entry.frameIndex}/${framePlan.totalFrames} downloaded ` +
  ...
  console.log(
    `[Step1] Download complete: ${successCount}/${framePlan.totalFrames} frames, ` +
  ...
  await fsp.writeFile(
    path.join(metadataDir, "camera_state.json"),
    JSON.stringify(cameraState, null, 2),
  );
  await fsp.writeFile(
  ...
  if (buildArchive) {
    ...
    execSync(
      `tar -czf "${archivePath}" -C "${metadataDir}" frames`,
      { stdio: "inherit" },
    );
  ...
  console.log(
    `[Step1] ✅ complete.json uploaded — Step 1 done.\n` +
