...
import { buildFramePlan, buildOverlayCoords, encodePolyline, } from "./utils/buildFramePlan.js";
...
const CONCURRENCY = 5;
function log(event) {
    process.stdout.write(JSON.stringify({ step: "step1_mcp", ...event }) + "\n");
}
...
function lookupAirport(code) {
    ...
    if (!coords)
        throw new Error(`[Step1MCP] Unknown airport code: ${code}`);
    return coords;
}
function getVisibleOverlayCoords(frameIndex, overlayCoords, diveFrames, flyoverFrames) {
...
// ── Main export ───────────────────────────────────────────────────────────
export async function downloadFramesMCP(mission, options) {
    const { slug } = mission;
    ...
    if (!MAPBOX_TOKEN)
        throw new Error("[Step1MCP] MAPBOX_ACCESS_TOKEN missing from environment");
    ...
    try {
        ...
        console.log(`[Step1MCP] Requesting road-snapped route: [${origin}] → [${destination}]`);
        ...
        console.log(`[Step1MCP] Frame plan ready: ${framePlan.totalFrames} frames, ` +
        ...
        await concurrentMap(framePlan.frames, CONCURRENCY, async (entry) => {
            ...
            const visibleOverlayCoords = getVisibleOverlayCoords(entry.frameIndex, overlayCoords, framePlan.diveFrames, framePlan.flyoverFrames);
            ...
            if (entry.frameIndex % 100 === 0) {
                console.log(`[Step1MCP] Frame ${entry.frameIndex}/${framePlan.totalFrames} rendered ` +
        ...
        console.log(`[Step1MCP] Render complete: ${successCount}/${framePlan.totalFrames} frames in ${Math.round(downloadMs / 1000)}s`);
        ...
        console.log(`[Step1MCP] ✅ complete.json uploaded — Step 1 (MCP) done.\n` +
