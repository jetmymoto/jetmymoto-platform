...
export type CameraMode = "drone_follow" | "chase" | "orbit" | "push_in" | "reveal";

export type RouteAnalysis = {
  curvaturePerSegment: number[];
...
function resolveActFrames(totalFrames: number): ActFrameCounts {
...
const TRANSITION_MAP: Record<string, number> = {
...
export const encodePolyline = (points: Array<[number, number]>): string => {
...
const buildCameraDrift = (frameCount: number): Array<[number, number]> => {
...
// ── Main export ───────────────────────────────────────────────────────────
function getModeForFrame(frameIndex: number, plan: CameraSegment[]): CameraMode {
...
function getTransitionContext(frameIndex: number, plan: CameraSegment[]) {
...
export function buildFramePlan(
  ...
  let overlayCoords = simplifiedOverlay.geometry.coordinates as Array<[number, number]>;
  ...
  let prevLngLat: [number, number] | null = null;
  let prevZoom: number | null = null;
  let prevBearing: number | null = null;
  let prevPitch: number | null = null;

  const calculateTarget = (mode: CameraMode, frameIndex: number, totalFrames: number) => {
    let targetLng = 0;
    let targetLat = 0;
    let targetZoom = 0;
    let targetBearing = 0;
    ...
    if (mode === "orbit") {
      ...
      const routeMidpoint = turf.along(
        cameraLine,
        cameraLengthKm / 2,
        { units: "kilometers" },
      ).geometry.coordinates;

      const pos = turf.destination(
        turf.point(routeMidpoint as [number, number]),
        radiusKm,
        angle,
        { units: "kilometers" },
      ).geometry.coordinates;
      ...
      if (frameIndex < diveFrames) {
        ...
        const lookAheadDist = Math.min(
          cameraLengthKm,
          currentDistance + Math.max(cameraLengthKm * 0.05, 5),
        );
        const lookAheadPoint = turf.along(
          cameraLine,
          lookAheadDist,
          { units: "kilometers" },
        ).geometry.coordinates;
        ...
        const finalBearing = turf.bearing(
  ...
  for (let i = 0; i < frameCount; i++) {
    ...
    if (prevLngLat === null) prevLngLat = [targetLng, targetLat];
    if (prevZoom === null) prevZoom = targetZoom;
    if (prevBearing === null) prevBearing = targetBearing;
    if (prevPitch === null) prevPitch = targetPitch;
...
export function buildFrameUrl(
  ...
  framePlan: Pick<FramePlanResult, "frames" | "diveFrames" | "flyoverFrames">,
  token: string,
): string {
  ...
  let visibleCoords: Array<[number, number]>;
 ...
 */
export function buildOverlayCoords(coords: Array<[number, number]>): Array<[number, number]> {
  const overlayLine = turf.lineString(coords);
  const simplified = turf.simplify(overlayLine, { tolerance: 0.00002, highQuality: true });
  let overlayCoords = simplified.geometry.coordinates as Array<[number, number]>;
  if (overlayCoords.length > 500) {
    const step = Math.ceil(overlayCoords.length / 500);
    overlayCoords = overlayCoords.filter((_, i) => i % step === 0 || i === overlayCoords.length - 1);
  }
 ...
 */
export function generateCameraPlan(
  routeAnalysis: RouteAnalysis,
  frameCount: number,
): CameraSegment[] {
  ...
  const numDataPoints = Math.max(
    routeAnalysis.curvaturePerSegment?.length || 0,
    routeAnalysis.elevationProfile?.length || 0,
    1,
  );
  ...
  let currentMode: CameraMode = "drone_follow";
  ...
  for (let f = startEndFrame; f < finalStartFrame; f++) {
    ...
    const isPeak =
      routeAnalysis.keyPoints?.peaks?.includes(dataIndex) ||
      (maxElevation > 0 && elevation >= maxElevation * 0.95);

    let targetMode: CameraMode = "drone_follow"; // 4. LONG STRAIGHT / LOW CURVATURE
