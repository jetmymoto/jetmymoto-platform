...
const AIRPORT_COORDS: Record<string, [number, number]> = airportCoordsRaw as unknown as Record<string, [number, number]>;

function lookupAirport(code: string): [number, number] | null {
...
function deduplicateSequential(coords: Array<[number, number]>): Array<[number, number]> {
 ...
 */
function synthesizeArc(start: [number, number], end: [number, number]): Array<[number, number]> {
 ...
 */
function applyCurve(coords: Array<[number, number]>): Array<[number, number]> {
  if (coords.length < 3) return coords;
  try {
    const line = turf.lineString(coords);
    const curved = turf.bezierSpline(line, { resolution: 10000, sharpness: 0.85 });
    const result = curved.geometry.coordinates as Array<[number, number]>;
...
function resolveCoordinates(mission: MissionInput): Array<[number, number]> {
  ...
  if (!insertionCode || !extractionCode) {
    throw new Error(
      "Cannot build route geometry: no coordinates and no airport codes provided."
    );
  }
  ...
  if (!start || !end) {
    throw new Error(
      `Unknown airport code: ${!start ? insertionCode : extractionCode}. Add it to airportCoords.json.`
    );
  }
...
export const buildRouteGeo = (mission: MissionInput): RouteGeo => {
