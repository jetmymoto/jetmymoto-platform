 ...
 */
export async function fetchRouteGeometry(options) {
    const token = (process.env.REMOTION_MAPBOX_TOKEN ||
        process.env.MAPBOX_TOKEN ||
        "").trim();
    if (!token) {
        throw new Error("Mapbox token is not configured. Set REMOTION_MAPBOX_TOKEN or MAPBOX_TOKEN.");
    }
    ...
    if (!json.routes || json.routes.length === 0) {
        throw new Error(`Mapbox returned no routes for ${coordinateString}`);
    }
    const fullCoordinates = json.routes[0].geometry.coordinates;
    if (!fullCoordinates || fullCoordinates.length < 5) {
        throw new Error(`Invalid Mapbox route: only ${fullCoordinates?.length ?? 0} points returned`);
    }
 ...
 */
export function resolveAirportCoord(code) {
    const coord = AIRPORT_COORDS[code];
    if (!coord) {
        throw new Error(`Unknown airport code: ${code}. Add it to airportCoords.json.`);
    }
