...
const MCP_SERVER_PATH = path.resolve(__dirname, "../../../.mcp-servers/mapbox/node_modules/@mapbox/mcp-server/dist/esm/index.js");
// ── Client ────────────────────────────────────────────────────────────────
export class MapboxMCPClient {
    ...
    async connect() {
        ...
        this.proc.on("exit", (code, signal) => {
            this.rejectPending(new Error(`[MCP] Server process exited (code=${code ?? "null"}, signal=${signal ?? "null"})`));
        });
        this.proc.on("close", (code, signal) => {
            this.rejectPending(new Error(`[MCP] Server process closed (code=${code ?? "null"}, signal=${signal ?? "null"})`));
        });
    ...
    async getRouteCoordinates(origin, destination) {
        ...
        if (resourceUriMatch) {
            ...
            const parsed = JSON.parse(resourceContent.text);
        ...
        try {
            ...
            throw new Error(`[MCP] directions_tool response is not parseable JSON. ` +
                `Response: "${text.substring(0, 300)}" — Original error: ${e.message}`);
        }
    }
    /**
    ...
    async renderFrame(input) {
        ...
        const resourceText = result.content?.find((c) => c.type === "text" && c.text?.includes("Resource URI:"));
