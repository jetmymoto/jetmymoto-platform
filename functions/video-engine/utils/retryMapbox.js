 ...
 */
export async function safeMapboxFetch(url, options = {}) {
    ...
    while (true) {
        ...
        try {
            ...
            if (genericAttempts >= GENERIC_RETRIES) {
                throw new Error(`[retryMapbox] ${label}: network error after ${genericAttempts} attempts — ${networkErr.message}`);
            }
            const delay = BASE_BACKOFF_MS * genericAttempts;
            console.warn(`[retryMapbox] ${label}: network error (attempt ${genericAttempts}/${GENERIC_RETRIES}), retrying in ${delay}ms — ${networkErr.message}`);
            await sleep(delay);
            continue;
        }
        if (response.ok) {
        ...
        if (response.status === 429) {
            ...
            console.warn(`[retryMapbox] ${label}: 429 rate-limited (attempt ${rateLimitAttempts}/${RATE_LIMIT_RETRIES}), backing off ${delay}ms`);
            await sleep(delay);
            continue;
        }
        ...
        if (response.status >= 500) {
            ...
            const delay = BASE_BACKOFF_MS * genericAttempts;
            console.warn(`[retryMapbox] ${label}: ${response.status} server error (attempt ${genericAttempts}/${GENERIC_RETRIES}), retrying in ${delay}ms`);
            await sleep(delay);
            continue;
        }
