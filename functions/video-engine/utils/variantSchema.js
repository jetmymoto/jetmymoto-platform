...
export const GRADE_FILTERS = {
     ...
     */
    balanced_bright: "eq=brightness=0.04:contrast=1.08:saturation=1.12:gamma=1.06," +
     ...
     */
    tactical_amber_bright: "eq=brightness=0.05:contrast=1.06:saturation=1.10:gamma=1.08," +
     ...
     */
    readable_bright: "eq=brightness=0.06:contrast=1.04:saturation=1.08:gamma=1.10," +
        "unsharp=5:5:0.6:3:3:0.2," +
        "vignette=PI/5," +
        "noise=alls=4:allf=t",
};
 ...
 */
export function isBrightGrade(grade) {
    return (grade === "balanced_bright" ||
...
function isCombinatorialVariant(obj) {
    if (typeof obj !== "object" || obj === null)
        return false;
    const keys = Object.keys(obj);
    return keys.some((k) => ["hook", "cta", "grade", "aspects"].includes(k));
}
function combinatorialToVariantConfig(c) {
    return {
        hookText: c.hook,
        ctaText: c.cta,
        gradePreset: c.grade,
 ...
 */
export async function parseVariantArg(variantArg) {
    ...
    if (variantArg.trimStart().startsWith("{")) {
        try {
            const parsed = JSON.parse(variantArg);
    ...
    try {
        ...
        throw new Error(`--variant="${variantArg}" is not a known preset, valid inline JSON, or readable JSON file`);
    }
}
